...
void PremiumCalculator_9_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void PremiumCalculator_9_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 67;}

uint get_number_of_components() {return 12;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 52;}
...
ctx->componentMemory[coffset].templateName = "PremiumCalculator";
ctx->componentMemory[coffset].signalStart = soffset;
...
GreaterEqThan_2_create(mySignalStart+11,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "vaccineCheck";
IsEqual_4_create(mySignalStart+60,9+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[1] = 9+ctx_index+1;
}
{
std::string new_cmp_name = "systolicCheck";
LessEqThan_7_create(mySignalStart+43,6+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[2] = 6+ctx_index+1;
}
{
std::string new_cmp_name = "diastolicCheck";
LessEqThan_8_create(mySignalStart+27,3+ctx_index+1,ctx,new_cmp_name,myId);
...
cmp_index_ref_load = 1;
Fr_mul(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0]); // line circom 64
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
cmp_index_ref_load = 2;
Fr_mul(&expaux[0],&signalValues[mySignalStart + 9],&ctx->signalValues[ctx->componentMemory[mySubcomponents[2]].signalStart + 0]); // line circom 67
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
cmp_index_ref_load = 3;
Fr_mul(&expaux[0],&signalValues[mySignalStart + 10],&ctx->signalValues[ctx->componentMemory[mySubcomponents[3]].signalStart + 0]); // line circom 69
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 4; i++){
...
PremiumCalculator_9_create(1,0,ctx,"main",0);
PremiumCalculator_9_run(0,ctx);
}
