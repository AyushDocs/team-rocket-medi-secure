...
async function uploadFileToPinata(localPath, metadata = {}) {
    ...
    try {
        if (!PINATA_API_KEY || !PINATA_SECRET_API_KEY) {
             throw new Error("PINATA_API_KEY or PINATA_SECRET_API_KEY is missing in .env");
        }
...
module.exports = async function(callback) {
  try {
    ...
    // --- 0. FUND ACCOUNTS IF THEY ARE EMPTY ---
    console.log("Checking account balances...");
    for (const acc of accounts) {
        const bal = await web3.eth.getBalance(acc);
        if (bal === "0") {
            console.log(`  > Funding ${acc} with 5 ETH from deployer...`);
            await web3.eth.sendTransaction({ from: deployer, to: acc, value: web3.utils.toWei("5", "ether") });
        }
    }
    ...
    for (let i = 0; i < patients.length; i++) {
        ...
        try {
            ...
            const exists = await patientContract.userExists(pAddr);
            if (!exists) {
                ...
                console.log(`  > Patient ${i+1} already registered.`);
            }
    ...
    for (let i = 0; i < doctors.length; i++) {
        ...
        try {
            ...
            const exists = await doctorContract.doctorExists(dAddr);
            if (!exists) {
                await doctorContract.registerDoctor(doctorProfiles[i].name, doctorProfiles[i].spec, doctorProfiles[i].hosp, { from: dAddr });
                console.log(`  > Registered Doctor ${i+1}`);
            } else {
                console.log(`  > Doctor ${i+1} already exists.`);
            }
    ...
    for(let i=0; i<hospitalAdmins.length; i++) {
        try {
            ...
            const hId = await hospitalContract.walletToHospitalId(hospitalAdmins[i]);
            if (hId.toString() === "0") {
                ...
                console.log(`  > Hospital already registered (ID: ${hId})`);
            }
