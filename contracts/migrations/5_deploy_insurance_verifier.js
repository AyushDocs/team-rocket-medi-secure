...
module.exports = async function (deployer) {
  ...
  console.log(`Insurance contract deployed at: ${insuranceInstance.address} at Network IDs: ${Object.keys(Insurance.networks || {}).join(", ")}`);

  // 2. Deploy Real Verifier
  await deployer.deploy(InsuranceVerifier);
  const verifierInstance = await InsuranceVerifier.deployed();
  console.log(`InsuranceVerifier deployed at: ${verifierInstance.address} at Network IDs: ${Object.keys(InsuranceVerifier.networks || {}).join(", ")}`);
