...
contract Insurance is ReentrancyGuard {
    ...
    struct Claim {
        ...
        uint256 cost;
        string evidenceHash; // IPFS CID of the clinical handoff report
    ...
    event ClaimSubmitted(uint256 indexed claimId, address indexed patient, string procedure);
    ...
    /**
     * @dev Step 1: Submit a clinical claim for reimbursement
     */
    function submitClaim(
        ...
        InsuranceRequest storage req = insuranceRequests[_requestId];
        require(req.isFinalized, "Policy must be finalized first");
        ...
        patientClaims[msg.sender].push(claimCount);
        providerClaims[req.provider].push(claimCount);

        emit ClaimSubmitted(claimCount, msg.sender, _procedureName);
    }

    /**
     * @dev Step 2: Process a claim (Insurance Provider only)
     */
    function processClaim(uint256 _claimId, string memory _status) public onlyInsurance {
        Claim storage clm = claims[_claimId];
        require(clm.provider == msg.sender, "Not your claim");
        require(keccak256(abi.encodePacked(clm.status)) == keccak256(abi.encodePacked("PENDING")), "Already processed");
    ...
    // --- History Getters ---
    function getPatientClaims(address _patient) public view returns (Claim[] memory) {
        uint256[] memory ids = patientClaims[_patient];
        Claim[] memory clms = new Claim[](ids.length);
        for(uint i = 0; i < ids.length; i++) {
            clms[i] = claims[ids[i]];
        }
        return clms;
    ...
    function getProviderClaims(address _provider) public view returns (Claim[] memory) {
        uint256[] memory ids = providerClaims[_provider];
        Claim[] memory clms = new Claim[](ids.length);
        for(uint i = 0; i < ids.length; i++) {
            clms[i] = claims[ids[i]];
        }
