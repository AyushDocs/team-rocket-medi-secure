...
export const triggerAccess = async (req, res) => {
  try {
    ...
    // Step 1: Log the emergency access event on-chain for HIPAA compliance
    try {
      const accounts = await web3.eth.getAccounts();
      const hospitalAdmin = accounts[0];
      console.log(`[AUDIT] Logging emergency access for patient ${patientId} by ${hospitalAdmin}`);
      
      await patientContract.methods.logEmergencyAccess(
        patientId, 
        "Emergency Baseline Fetch via PremKatha AI Engine (Break-Glass Enabled)"
      ).send({ from: hospitalAdmin, gas: 300000 });
      
      console.log(`[AUDIT] Access record locked on blockchain`);
    } catch (auditErr) {
      console.warn("[AUDIT] Failed to record on-chain access log:", auditErr.message);
      // We continue with access even if audit log fails for safety in a real emergency
    }
